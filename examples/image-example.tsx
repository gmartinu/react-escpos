/**
 * Image Receipt Example
 *
 * This example demonstrates how to use images (logos, QR codes, etc.) in receipts
 * Images are provided as base64 strings or data URIs
 */

import React from "react";
import { Document, Page, View, Text, Image } from "@react-pdf/renderer";
import { convertToESCPOS } from "../dist/index.js";
import fs from "fs";

// Sample base64 image - A 100x100 test pattern with border and X shape
// In production, you would use your actual company logo as base64
const SAMPLE_LOGO_BASE64 =
  "";

// Define receipt data interface
interface ReceiptWithImageData {
  logo: string; // base64 or data URI
  storeName: string;
  storeAddress: string;
  date: string;
  items: Array<{
    name: string;
    quantity: number;
    price: number;
  }>;
  total: number;
}

// Receipt component with image
const ReceiptWithImage: React.FC<{ data: ReceiptWithImageData }> = ({
  data,
}) => (
  <Document>
    <Page>
      <View>
        {/* Logo Image */}
        <View style={{ textAlign: "center" }}>
          <Image
            src={`data:image/png;base64,${data.logo}`}
            style={{ width: 200, height: 200 }}
          />
        </View>

        {/* Store Header */}
        <Text style={{ textAlign: "center", fontWeight: "bold", fontSize: 20 }}>
          {data.storeName}
        </Text>
        <Text style={{ textAlign: "center" }}>{data.storeAddress}</Text>
        <Text style={{ textAlign: "center" }}>{data.date}</Text>

        <View style={{ borderBottom: "1px solid black" }} />

        {/* Items */}
        {data.items.map((item, index) => (
          <View key={index}>
            <View
              style={{ flexDirection: "row", justifyContent: "space-between" }}
            >
              <Text>{item.name}</Text>
              <Text>${item.price.toFixed(2)}</Text>
            </View>
            <Text> Qty: {item.quantity}</Text>
          </View>
        ))}

        <View style={{ borderTop: "1px solid black" }} />

        {/* Total */}
        <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
          <Text style={{ fontWeight: "bold", fontSize: 18 }}>TOTAL:</Text>
          <Text style={{ fontWeight: "bold", fontSize: 18 }}>
            ${data.total.toFixed(2)}
          </Text>
        </View>

        {/* Footer */}
        <Text style={{ textAlign: "center", fontSize: 12 }}>
          Thank you for your purchase!
        </Text>
      </View>
    </Page>
  </Document>
);

// Usage example
async function printReceiptWithImage() {
  const receiptData: ReceiptWithImageData = {
    logo: SAMPLE_LOGO_BASE64,
    storeName: "My Awesome Store",
    storeAddress: "123 Main St, City, State 12345",
    date: new Date().toLocaleString(),
    items: [
      { name: "Product A", quantity: 2, price: 19.99 },
      { name: "Product B", quantity: 1, price: 29.99 },
      { name: "Product C", quantity: 3, price: 9.99 },
    ],
    total: 79.96,
  };

  try {
    console.log("🖨️  Generating receipt with image...");

    // Convert to ESC/POS
    const buffer = await convertToESCPOS(
      <ReceiptWithImage data={receiptData} />,
      {
        paperWidth: 48, // 80mm thermal printer (48 characters)
        encoding: "utf-8",
        cut: "full", // Full cut
        feedBeforeCut: 3, // Feed 3 lines before cut
        debug: false,
      }
    );

    // Save to file for testing
    fs.writeFileSync("receipt-with-image.bin", buffer);
    console.log("✅ Receipt with image generated successfully!");
    console.log(`📄 Saved to: receipt-with-image.bin`);
    console.log(`📏 Buffer size: ${buffer.length} bytes`);
    console.log("");
    console.log("To test on a thermal printer:");
    console.log(
      "  - Copy receipt-with-image.bin to your printer via USB/Network"
    );
    console.log(
      "  - Windows: copy /b receipt-with-image.bin \\\\\\\\localhost\\\\PrinterName"
    );
    console.log("  - Linux/Mac: cat receipt-with-image.bin > /dev/usb/lp0");
    console.log("");
  } catch (error) {
    console.error("❌ Error generating receipt:", error);
  }
}

// Run the example
printReceiptWithImage();
