 ...
 */
export const transmitRealtimeStatus = (n: 1 | 2 | 3 | 4): number[] => [
  DLE,
  0x04,
  n,
];
 ...
 */
export const selectInternationalCharset = (
  charset: InternationalCharset
): number[] => [ESC, 0x52, charset];
 ...
 * Vertical dots = y × 8
 */
export const defineDownloadedBitImage = (
  x: number,
  y: number,
  data: number[]
): number[] => {
 ...
 * m: Print mode (0-3)
 */
export const printDownloadedBitImage = (
 ...
 */
export const printNVBitImage = (
  imageNumber: number,
  mode: DownloadedImageMode
): number[] => [FS, 0x70, Math.max(1, Math.min(255, imageNumber)), mode];
 ...
 */
export const printBarcodeWithLength = (
  type: number,
  data: string
): number[] => {
  const bytes = data.split("").map((c) => c.charCodeAt(0));
...
export const printBarcodeCODABAR = (data: string): number[] => {
  if (!/^[A-D][0-9A-D $+\-.\/:]*[A-D]$/.test(data)) {
    throw new Error(
      "CODABAR must start and end with A-D, and contain 0-9, A-D, $+-./:"
    );
  }
 ...
 */
export const selectCharacterSize = (
 ...
 */
export const requestStatus = (type: StatusType): number[] => [GS, 0x72, type];
